...
class CovoitController extends Controller
{
    ...
    public function mes_trajetsAction(Request $request)
    {
      ...
      {
          $mes_trajets[] = $trajet;
          $demandes = $em->getRepository('GSBCovoitBundle:Demande')->findBy(array('trajetId' => $trajet->getId()));
          $listDemandes[] = $demandes;
      }
      if (NULL === $mes_trajets) {
        throw new NotFoundHttpException("Vous n'avez pas encore proposé de trajet.");
      }
      return $this->render('GSBCovoitBundle:Covoit:mes_trajets.html.twig',
                          array('listTrajets'  => $mes_trajets,
                                ...
                                'listDemandes'=>$listDemandes,
                                'i' => $i,
                                'demandesDuTrajet' => $demandesDuTrajet));
    }
